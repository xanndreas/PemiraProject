    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.categories.update", [$category->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('open_date') ? 'is-invalid' : '' }}" type="text" name="open_date" id="open_date" value="{{ old('open_date', $category->open_date) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('close_date') ? 'is-invalid' : '' }}" type="text" name="close_date" id="close_date" value="{{ old('close_date', $category->close_date) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('candidates') ? 'is-invalid' : '' }}" name="candidates[]" id="candidates" multiple required>
                    @foreach($candidates as $id => $candidate)
                        <option value="{{ $id }}" {{ (in_array($id, old('candidates', [])) || $category->candidates->contains($id)) ? 'selected' : '' }}>{{ $candidate }}</option>
                    @endforeach
                </select>
