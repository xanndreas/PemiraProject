    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.candidates.update", [$candidate->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jurusan') ? 'is-invalid' : '' }}" name="jurusan_id" id="jurusan_id" required>
                    @foreach($jurusans as $id => $jurusan)
                        <option value="{{ $id }}" {{ ($candidate->jurusan ? $candidate->jurusan->id : old('jurusan_id')) == $id ? 'selected' : '' }}>{{ $jurusan }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('sebagai') ? 'is-invalid' : '' }}" name="sebagai" id="sebagai">
                    ...
                    @foreach(App\Candidate::SEBAGAI_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('sebagai', $candidate->sebagai) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('organization') ? 'is-invalid' : '' }}" type="text" name="organization" id="organization" value="{{ old('organization', $candidate->organization) }}" required>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control ckeditor {{ $errors->has('visimisi') ? 'is-invalid' : '' }}" name="visimisi" id="visimisi">{!! old('visimisi', $candidate->visimisi) !!}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link') ? 'is-invalid' : '' }}" type="text" name="link" id="link" value="{{ old('link', $candidate->link) }}" required>
                @if($errors->has('link'))
                    <div class="invalid-feedback">
                        {{ $errors->first('link') }}
                    </div>
