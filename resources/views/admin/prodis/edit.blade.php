    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.prodis.update", [$prodi->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $prodi->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $prodi->description) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('jumlah_mahasiswa') ? 'is-invalid' : '' }}" type="number" name="jumlah_mahasiswa" id="jumlah_mahasiswa" value="{{ old('jumlah_mahasiswa', $prodi->jumlah_mahasiswa) }}" step="1" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('jurusan') ? 'is-invalid' : '' }}" name="jurusan_id" id="jurusan_id" required>
                    @foreach($jurusans as $id => $jurusan)
                        <option value="{{ $id }}" {{ ($prodi->jurusan ? $prodi->jurusan->id : old('jurusan_id')) == $id ? 'selected' : '' }}>{{ $jurusan }}</option>
                    @endforeach
                </select>
